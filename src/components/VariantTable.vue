...
// NEW: Get missing databases
const getMissingDatabases = (sourcesFound) => {
  const allDatabases = ['dbSNP', 'ClinVar', 'dbNSFP']
  return allDatabases.filter(db => !sourcesFound?.includes(db))
}
</script>
          ...
          <tbody class="bg-white divide-y divide-slate-200">
            <template v-for="item in paginatedVariants" :key="item.rsid">
              ...
              <tr class="hover:bg-slate-50 transition-colors cursor-pointer" @click="toggleRow(item.rsid)">
                <td class="px-6 py-4">
                  <component 
                    :is="expandedRows.has(item.rsid) ? ChevronUpIcon : ChevronDownIcon" 
                ...
                <td class="px-6 py-4">
                  <div class="flex flex-wrap gap-1">
                    <!-- Present Databases (Green) -->
                    <span 
                      v-for="src in item.payload?.sources_found" 
                      :key="src" 
                    ...
                    <!-- Missing Databases (Red) -->
                    <span 
                      v-for="missing in getMissingDatabases(item.payload?.sources_found)" 
                      :key="missing" 
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div v-if="item.payload?.data?.common">
                      ...
                      <div class="grid grid-cols-2 gap-2">
                        <div 
                          v-for="(val, key) in getFrequencyData(item.payload.data.common)" 
                          :key="key" 
                    ...
                    <div v-if="item.payload?.data?.genes?.length > 0">
                      ...
                      <div class="flex flex-wrap gap-2">
                        <span 
                          v-for="gene in item.payload.data.genes" 
                          :key="gene"
                    ...
                    <!-- Functional Predictions (if available) -->
